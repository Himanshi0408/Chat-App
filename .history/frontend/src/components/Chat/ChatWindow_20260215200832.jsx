...
export default function ChatWindow({ chatWith }) {
  ...
  const { messages, setMessagesForChat, addMessage, replaceOptimisticMessage, updateOnlineUsers } =
  ...
  const tempMessageIdsRef = useRef(new Set()); // Track optimistic message IDs
  // FETCH CHAT HISTORY
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleReceiveMessage = (msg) => {
      ...
      console.log(`Checking: sender=${msg.sender._id}, receiver=${msg.receiver._id}`);
      console.log(`Current chat: with=${chatWith._id}, user=${user._id}`);
      
      if (
        (msg.sender._id === chatWith._id && msg.receiver._id === user._id) ||
        (msg.sender._id === user._id && msg.receiver._id === chatWith._id)
      ) {
        console.log("✓ Message is for current chat, adding...");
        
        // Check if this is already in our UI as optimistic message
        const isOptimisticMsg = messages.some(m => m.tempId);
        if (isOptimisticMsg) {
          console.log("Optimistic message found, updating with server data");
          // Update any optimistic message with actual server data
          setMessages(prev => [
            ...prev.filter(m => !m.tempId),
            msg
          ]);
        } else {
  ...
  }, [chatWith?._id, user?._id, addMessage, messages]);
  ...
  const sendMessage = async (text) => {
    ...
    // 1️⃣ OPTIMISTIC UPDATE: Add message IMMEDIATELY to UI
